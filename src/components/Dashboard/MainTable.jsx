...
const MainTable = ({
    ...
    if (loading) {
        return (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center">
                ...
                <p className="text-slate-500 font-bold animate-pulse uppercase tracking-widest text-xs">Syncing Performance Data...</p>
            </div>
        );
    }
    ...
    if (!processedData || processedData.length === 0) {
        return (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center text-slate-400 font-medium border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        {processedData.map((row, idx) => (
                            <tr key={idx} className={`group transition-colors ${row.isMisc ? 'bg-orange-50 dark:bg-orange-950/20 font-semibold' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                                ...
                                {columns.map(col => {
                                    ...
                                    else if (col.type === 'percentage' || col.type === 'percentage_trend') displayVal = formatPercent(displayVal);
                                    else if (col.type === 'number') displayVal = formatNumber(displayVal);

                                    if (col.key === 'paceToGoal') {
                                        colorClass = (row[col.key] || 0) >= 100 ? "text-green-600 dark:text-green-400 font-black" :
                                            (row[col.key] || 0) >= 90 ? "text-blue-600 dark:text-blue-400 font-bold" :
                                                (row[col.key] || 0) >= 70 ? "text-slate-600 dark:text-slate-300" : "text-red-600 dark:text-red-400";
                                    }

                                    if (col.key === 'grossProfitPct') {
                                        const target = adminSettings?.locationGoals?.[row.strDepartment]?.profitGoal || 25;
                                        colorClass = (row[col.key] || 0) >= target ? "text-green-600 dark:text-green-400 font-bold" : "text-orange-600 dark:text-orange-400";
                                    }
                                    ...
                                    if (col.key === 'curQuoteTotals') {
                                        const act = row.curQuoteTotals || 0;
                                        const goal = row.toDateEstGoal || 0;
                                        const color = act >= goal ? "text-green-600 dark:text-green-400" : "text-slate-400";
                                        return (
                                            <td key={col.key} className="p-1.5 text-right text-sm border-r border-slate-50 dark:border-slate-800 last:border-0">
                                                <div className="flex flex-col items-end">
                                                    <div className="font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(act)}</div>
                                                    <div className="text-[9px] font-bold text-slate-400 uppercase">Goal: {formatCurrency(goal)}</div>
                                                </div>
                                            </td>
                                        );
                                    }
